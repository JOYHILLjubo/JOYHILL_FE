...
- **바가 차지하는 높이는 아래에서부터 77px**(아래 여백 6 + 바 높이 71) + safe-area. 페이지들의 하단 패딩은 `calc(96px + env(safe-area-inset-bottom, 0px))`가 표준이다. 새 화면을 만들 때 이 값을 쓸 것. **바 크기나 위치를 바꾸면 이 패딩과 `AttendancePage`의 저장 알약 offset(현재 `bottom: 85px`, 패딩 97/157/203)도 같이 맞춰야 한다** — 안 맞추면 목록 맨 아래 항목이 가려진다.
