...
**Enter는 execCommand 실패에 대비한 폴백이 반드시 있어야 한다 (2026-08-25)**: `insertLineBreak`는 비표준이라 지원하지 않는 브라우저가 있는데, 이 에디터는 Enter를 `preventDefault`로 먼저 가로채기 때문에 명령이 실패하면 **엔터키가 통째로 죽는다**. `execCommand` 반환값을 보고 실패하면 `insertLineBreakManually()`로 직접 `<br>`을 넣는다. 한글 조합 판정은 `isComposing` **과 `keyCode === 229`를 둘 다** 본다(안드로이드 키보드 일부가 `isComposing`을 안 채운다).
...
- **바가 차지하는 높이는 아래에서부터 70px**(아래 여백 10 + 바 높이 60) + safe-area. 페이지들의 하단 패딩은 `calc(88px + env(safe-area-inset-bottom, 0px))`가 표준이다(예전 80px에서 올림). 새 화면을 만들 때 이 값을 쓸 것.
