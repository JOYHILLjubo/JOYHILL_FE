...
function getApiErrorMessage(result, fallbackMessage) {
  ...
  if (result.response.status === 403) {
    return result.payload?.error?.message ?? '기도제목을 조회할 권한이 없습니다.'
  }
...
export default function PrayerPageConnected() {
  ...
  const personalPrayers = useMemo(() => {
    ...
    monthlyPrayers
      .filter((prayer) => prayer.type === 'personal' && prayer.week === selectedWeek.week)
      .forEach((prayer) => {
        const key = prayer.userId ?? `guest-${prayer.id}`
        const existing = byUser.get(key)

        if (!existing || Number(prayer.id ?? 0) > Number(existing.id ?? 0)) {
          byUser.set(key, prayer)
        }
      })
  ...
  const commonPrayer = useMemo(() => {
    return (
      monthlyPrayers.find(
        (prayer) =>
          prayer.type === 'common' &&
          prayer.month === selectedWeek.month &&
          prayer.year === selectedWeek.year,
      ) ?? null
    )
  }, [monthlyPrayers, selectedWeek.month, selectedWeek.year])

  const myPrayer = useMemo(() => {
    return personalPrayers.find((prayer) => prayer.userId === user?.id) ?? null
  ...
  useEffect(() => {
    ...
    const loadPrayers = async () => {
      ...
      try {
        ...
        const nextPrayers = Array.isArray(data)
          ? data.map((item) => normalizePrayer(item, user))
          : []

        setMonthlyPrayers(nextPrayers)
      } catch (error) {
        const message =
          error instanceof Error ? error.message : '기도제목을 불러오지 못했습니다.'
  ...
  }, [user?.fam, selectedWeek.year, selectedWeek.month, reloadKey])

  return (
    <div className="pb-20">
      <div className="px-5 pt-4 pb-3">
        <p className="text-lg font-medium">팸 기도제목</p>
        <p className="text-xs text-gray-500 mt-0.5">{user?.fam || '소속 팸 없음'}</p>
      </div>
        ...
        {showWeekPicker && (
          <div className="mt-1 border border-gray-300 rounded-lg bg-white max-h-48 overflow-y-auto">
            {weekOptions.map((option) => {
              const isSelected =
                option.year === selectedWeek.year &&
                option.month === selectedWeek.month &&
                option.week === selectedWeek.week
        ...
        <div className="bg-primary-light rounded-xl p-4">
          <div className="flex justify-between items-start mb-1">
            <p className="text-[13px] font-medium text-primary-hover">
              {selectedWeek.month}월 공동 기도제목
            </p>
            {isLeaderOrAbove && (
              <button
                onClick={() =>
                  navigate(
                    `/prayer/common/write?year=${selectedWeek.year}&month=${selectedWeek.month}${
          ...
          {isLoading ? (
            <p className="text-[13px] text-primary-hover opacity-70 mt-1">
              공동 기도제목을 불러오는 중입니다.
            </p>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-medium">
                    ...
                    {prayer.userId === user?.id && (
                      <span className="ml-1 text-[11px] text-primary">(나)</span>
                    )}
                  </p>
