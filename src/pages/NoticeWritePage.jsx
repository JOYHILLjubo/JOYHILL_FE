  ...
  if (!canWriteNotice) {
    return (
      <div className="flex flex-col items-center justify-center h-screen">
        ...
        <button onClick={() => navigate('/notice')} className="mt-3 text-xs text-primary bg-primary-light px-4 py-2 rounded-full border-none cursor-pointer">
          돌아가기
        </button>
      </div>
    )
  ...
  return (
    <div className="pb-28">
      ...
      <div className="flex items-center gap-3 px-5 pt-4 pb-3 border-b border-gray-300">
        ...
        <p className="text-base font-medium">공지사항 등록</p>
      </div>
      ...
      <div className="px-5 pt-4 flex flex-col gap-5">
        ...
        {/* 태그 */}
        <div>
          <p className="text-xs text-gray-500 mb-1.5">태그 <span className="text-danger">*</span><span className="ml-1 font-normal">(중복 선택 가능)</span></p>
        ...
        <div>
          ...
          <label className="flex items-center gap-2.5 border border-dashed border-gray-300 rounded-lg px-3 py-3 cursor-pointer hover:bg-gray-100 transition-colors">
            ...
            <span className="text-sm text-gray-500 flex-1 truncate">{file ? file.name : '파일을 선택하세요'}</span>
            {file && (
