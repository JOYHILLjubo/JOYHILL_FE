...
export default function AttendanceHistoryPage() {
  ...
  const toggle = (memberId, dateKey, type) => {
    ...
    setAttendance((prev) => ({
      ...
      [memberId]: {
        ...prev[memberId],
        [dateKey]: { ...prev[memberId]?.[dateKey], [type]: prev[memberId]?.[dateKey]?.[type] === true ? null : true },
      },
    }))
  }

  const getRecord = (memberId, dateKey) => attendance[memberId]?.[dateKey] || { worship: null, fam: null }

  const handleSave = async () => {
    if (!famName) { setSaveError('소속 팸 정보가 없어 저장할 수 없습니다.'); return }
    if (famMembers.length === 0) { setSaveError('저장할 팸원이 없습니다.'); return }
