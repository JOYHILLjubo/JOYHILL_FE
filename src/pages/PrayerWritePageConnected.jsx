...
function getApiErrorMessage(result, fallbackMessage) {
  ...
  if (result.response.status === 403) {
    return result.payload?.error?.message ?? '기도제목을 저장할 권한이 없습니다.'
  }

  if (result.response.status === 404) {
    return result.payload?.error?.message ?? '수정할 기도제목을 찾을 수 없습니다.'
  }
...
export default function PrayerWritePageConnected() {
  ...
  const editingPrayer = useMemo(
    () =>
      location.state?.mode === 'edit'
        ? normalizePrayerState(location.state?.prayer)
        : null,
    [location.state],
  )
  ...
  const handleSubmit = async () => {
    ...
    if (!trimmed) {
      setSubmitError('기도제목을 입력해주세요.')
      return
    }

    if (trimmed.length > 200) {
      setSubmitError('기도제목은 200자 이하로 작성해주세요.')
      return
    }
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : '기도제목 저장에 실패했습니다.'
      ...
      <div className="flex items-center gap-2 px-5 pt-4 pb-2">
        ...
        <div>
          ...
          <p className="text-[11px] text-gray-500">
            {user?.fam || '소속 팸 없음'} · {month}월 {week}주차
          </p>
        </div>
      </div>
